...
def SRGAN_g(t_image, avg_image, is_train=False, reuse=False):
    ...
    with tf.variable_scope("SRGAN_g", reuse=reuse) as vs:
        ...
        res = InputLayer(avg_image, name='g_res')
        n = Conv2d(n, 64, (7, 7), (1, 1), act=tf.nn.relu, padding='SAME', W_init=w_init, name='n64s1/c')
        n = InstanceNormLayer(n, act=tf.nn.relu, name='n64s1/c/i')

        # 2 downsampling block
        for i in range(2):
            mult = 2**(i+1)
            nn = Conv2d(n, 64*mult, (3, 3), (2, 2), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n%ss2/c' % (64*mult))
            nn = InstanceNormLayer(nn, act=tf.nn.relu, name='n%ss2/i' % (64*mult))
            n = nn

        # B residual blocks
        for i in range(9):
            nn = Conv2d(n, 256, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c1/%s' % i)
            nn = InstanceNormLayer(nn, act=tf.nn.relu, name='n64s1/i1/%s' % i)
            nn = Conv2d(nn, 256, (3, 3), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n64s1/c2/%s' % i)
            nn = InstanceNormLayer(nn, act=tf.nn.relu, name='n64s1/i2/%s' % i)
        ...
        for i in range(2):
            mult = 2**(2-i)
            nn = DeConv2d(n, int(64*mult/2), (3, 3), (2, 2), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='n%ss2/dc' % (64*mult/2))
            nn = InstanceNormLayer(nn, act=tf.nn.relu, name='n%ss2/c/i' % (64*mult))
            n = nn

        n = Conv2d(n, 3, (7, 7), (1, 1), act=tf.nn.tanh, padding='SAME', W_init=w_init, name='out')
        n = ElementwiseLayer([n, res], tf.add, act=tf.identity, name='global_residual_add')
        return n
...
def SRGAN_d(input_images, is_train=True, reuse=False):
    ...
    with tf.variable_scope("SRGAN_d", reuse=reuse):
        ...
        n = Conv2d(net_in, df_dim, (4, 4), (2, 2), act=lrelu, padding='SAME', W_init=w_init, name='h0/c')

        nf_mult, nf_mult_prev = 1, 1
        for i in range(nf_layers):
            nf_mult_prev, nf_mult = nf_mult, min(2**i, 8)
            nn = Conv2d(n, df_dim * nf_mult, (4, 4), (2, 2), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='h%s/c' % (i+1))
            nn = InstanceNormLayer(nn, act=lrelu, name='h%s/i' % i)
            n = nn

        nf_mult_prev, nf_mult = nf_mult, min(2**nf_layers, 8)
        n = Conv2d(n, df_dim * nf_mult, (4, 4), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='h%s/c' % (nf_layers+1))
        n = InstanceNormLayer(n, act=lrelu, name='h%s/i' % (nf_layers+1))
        n = Conv2d(n, 1, (4, 4), (1, 1), act=None, padding='SAME', W_init=w_init, b_init=b_init, name='h%s/c' % (nf_layers+2))
        #net_hf = FlattenLayer(n, name='ho/flatten')
        #net_hd = DenseLayer(net_hf, n_units=1024, act=tf.nn.tanh, W_init=w_init, name='ho/dense/th')
        #net_ho = DenseLayer(net_hd, n_units=1, act=tf.identity, W_init=w_init, name='ho/dense/id')
        logits = n.outputs
        n.outputs = tf.nn.sigmoid(n.outputs)
